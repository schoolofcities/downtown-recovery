<script>
	...
	if (canvasElement) {
		...
		type: 'bar',
		data: {
			labels: ['rq_hdbscan', 'rq_comm'],
			datasets: [
			{
				label: selectedCity,
				data: [rqHdbscan, rqComm],
				backgroundColor: ['blue', 'green'], // Customize the colors as needed
			},
			],
		},
		options: {
	...
	// Update the map and bar chart when the selected city changes
	function updateMap() {
