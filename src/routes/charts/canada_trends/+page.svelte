<script>
	...
	let selection = {
		...
		day1_num: 1,
		day2_num: 30,
		update_date: '2025-10-17' // change this to whenever website is updated
	};
	...
	onMount(() => {
		loadData();
		filteredCities = cities.filter((item) => item.region === 'Canada').map((item) => item.city);
	});
...
<main>
	<div class="text">
		...
		<p>
			By <a href="https://schoolofcities.utoronto.ca/people/karen-chapple/">Karen Chapple</a>,
			<a href="https://www.urbandisplacement.org/team/julia-greenberg/">Julia Greenberg</a>,
			<a href="https://schoolofcities.utoronto.ca/people/jeff-allen/">Jeff Allen</a>,
		...
		<p>
			Data on cell phone activity (a.k.a. footfall) trends for the last year provide a picture
			of how Canadian downtowns are faring. We look here at year-over-year ({selection.year2} vs.
			{selection.year1}) trends.
		</p>
		<p>
			The solid line represents the number of daily unique visitors in the downtown area. The
			dotted line provides a baseline of the average level of activity in {selection.monthName}
			{selection.year1}, allowing for comparison to subsequent months. When the solid line
			extends above the dotted baseline, downtown activity is greater compared to in {selection.monthName}
			{selection.year1}. When it dips below the dotted line, activity is on a downswing.
		</p>
		<!-- <h5>
			Key Findings:
		</h5> -->
		...
		<p>
			Note: Trends are based on data from Spectus, but use different cell phone data providers
			from our rankings analysis. The trendlines measure the average level of activity over
			the course of the year, while the ranking metric shows the percent difference in the
			average number of unique visitors in {selection.year2} versus the same month in {selection.year1}.
		</p>
	...
	<div class="text">
		...
		<p>
			...
			<a href="/trend_canada_sep1_2024_to_sep30_2025.csv">from this link</a>. The data on the
			charts are based on the `normalized_distinct_clean` column, which pertains to the number
			of unique daily visitors normalized by the total number in the metro area. The
			trend-line and summary statistics shown are calculated in JavaScript (code is on
			<a
